...
public class GenerateOutput {
	...
	private PrintWriter protienRefWriter;
	private PrintWriter protienAltWriter;
	...
	private void cleanup() {
		refDNA.close();
		altDNA.close();
		protienRefWriter.close();
		protienAltWriter.close();
	...
	private void setPrinters() throws FileNotFoundException, URISyntaxException {
		...
		File RefOutput = new File(FileHelp.getRefProtienFile(fileName));
		File AltOutput = new File(FileHelp.getAltProtienFile(fileName));

		protienRefWriter = new PrintWriter(RefOutput);
		protienAltWriter = new PrintWriter(AltOutput);
	...
	private void processGene(Transcript gene) {
		...
		for (int i = 0; i < aachangePos.size(); i++) {
			protienRefWriter.print(fastaHeader);
			protienRefWriter.println(aachangePos.get(i) + 1);
			int start = getStartIndex(aachangePos.get(i));
			int end = getEndIndex(refProtein, aachangePos.get(i));

			protienRefWriter.println(refProtein.substring(start, end + 1));

			protienAltWriter.print(fastaHeader);
			protienAltWriter.println(aachangePos.get(i) + 1);
			protienAltWriter.println(altProtein.substring(start, end + 1));
